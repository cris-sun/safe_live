...
def create_map(age, sex, area, day_occurred, month_occurred, year_occurred):
    ...
    def style_function(feature):
        if feature['properties'].get('PREC', 'Unknown Area') == area:
            ...
            return {
                'fillColor': '#ffaf00',  # A bright color for the selected area
                'color': 'red',  # Border color for the selected area
                'weight': 3,
                'dashArray': '1, 1',
                'fillOpacity': 0.7,
            }
...
def main():
    ...
    with col2:
        ...
        #crime_data = [1 : 2, 2, 3 : 4,4 : 5, 5 : 6 , 6 : 7]
        #crime_data = pd.read_csv('raw_data/data.csv')
        #st.dataframe(crime_data)

        #chart = alt.Chart(crime_data).mark_bar().encode(
        #    x=crime_data[0],
        #    y=crime_data[1],
        #    tooltip=['Column', 'Colimn']
        #).interactive()
